...
def within_image_loss(img: torch.Tensor, img_jittered: torch.Tensor, masks: torch.Tensor,
    ...
    the first dimension is a label (so multilabel problem, not multiclass).
    :param tau Temperature parameter
    :param epsilon: for numerical stability (not in the paper)
    :param kwargs: for unloved variables </3
    :return:
    """
    img_flatten = img.flatten(1, 2).transpose(0, 1) # DxN -> NxD
    N, D = img_flatten.shape
    jit_flatten = img_jittered.flatten(1, 2) # DxN
    quadratic_matrix = torch.exp(torch.mm(img_flatten, jit_flatten) / tau)  # NxN: e_{pq}^{IÎ}
    ...
    for l in masks:
        ...
        loss += -((torch.log(quad_L / normalization_matrix[l]+epsilon)).sum(1) / (l.sum())).sum()
    return loss/N
...
def cross_image_loss(img_a: torch.Tensor, img_a_jit: torch.Tensor,
    ...
    within_img_quadratic = torch.exp(torch.matmul(img_a_flatten, img_a_jit_flatten) / tau)  # NxN: e_{pq}^{IÎ}
    cross_img_quadratic = torch.exp(torch.matmul(img_a_flatten, img_b_flatten) / tau)  # NxN: e_{pq}^{IĴ}
    normalization_within_img = within_img_quadratic.sum(1)  # Nx1 \Sigma e_{pq}^{IÎ}
    ...
    for l_a, l_b in zip(masks_a, masks_b):
        ...
        if not l_a.any() or not l_b.any():
            continue
        e_IJ = cross_img_quadratic[l_a, :][:, l_b]
        l1 = -torch.log(within_img_quadratic[l_a][:, l_a] / (normalization_within_img[l_a] + e_IJ.sum(1)+epsilon)) / (
                l_a.sum() + l_b.sum())
        l1 = l1.sum(1)
        l2 = -(torch.log(e_IJ / torch.unsqueeze(normalization_within_img[l_a] + e_IJ.sum(1)+epsilon, 1))) / (l_a.sum() + l_b.sum())
        l2 = l2.sum(1)
        loss += (l1 + l2).sum()
    return loss/N
...
def random_jitter(batch_img, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225),
    ...
    if proba < jitter_proba:
        batch_img = (batch_img - batch_img.min(dim=0, keepdim=True))/(batch_img.max(dim=0, keepdim=True)
                                                                      - batch_img.min(dim=0, keepdim=True))  # Bring back to the [0,1] range.
        proba = max_hue*proba
        batch_img = TF.adjust_hue(batch_img, proba)
        batch_img = batch_img - torch.Tensor(mean).unsqueeze(0).unsqueeze(2).unsqueeze(3).to(batch_img)
        batch_img = batch_img/torch.Tensor(std).unsqueeze(0).unsqueeze(2).unsqueeze(3).to(batch_img)
    return batch_img
...
def contrastive_loss_from_batch(model, imgs, gts, loss_type=ContrastiveLoss.SINGLE_IMAGE, **c):
    ...
    bg_class = (torch.argmax(gts, 1, keepdim=True) < 1).long()
